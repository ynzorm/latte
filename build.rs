...
mod driver_date_utils {
    ...
    pub fn get_driver_commit_date_from_github(sha: &str) -> Option<String> {
        let client = Client::builder()
            .timeout(FETCH_TIMEOUT)
            .user_agent("latte (github.com/scylladb/latte)")
            .build()
            .ok()?;
        let resp = client
            .get(format!(
...
mod driver_date_utils {
    ...
    pub fn get_driver_commit_date_from_github(_sha: &str) -> Option<String> {
        Some(UNKNOWN_EXTENDED.into())
    }
}
...
fn print_version(out_dir: OsString, commit_date: String, sha: String, lockfile: Lockfile) {
    let error_msg = format!("{DRIVER_PKG_NAME} package not found in Cargo.lock");
    let driver_pkg = lockfile
        .packages
        .iter()
        .find(|p| p.name.as_str() == DRIVER_PKG_NAME)
        .expect(&error_msg);
    let driver_source = driver_pkg
        .source
        .as_ref()
        .map(|s| s.to_string())
        .unwrap_or_default();
    let mut driver_version = driver_pkg.version.to_string();
    let (driver_commit_date, driver_sha) = if driver_source.is_empty() {
        driver_version = format!("{driver_version}-dev");
        (
            Utc::now().to_rfc3339_opts(chrono::SecondsFormat::Secs, true),
            UNKNOWN.into(),
        )
    } else if driver_source.contains("git+") {
        driver_version = format!("{driver_version}-dev");
        let commit_sha = driver_source
            .split('#')
            .nth(1)
            .unwrap_or(UNKNOWN)
            .to_string();
        (
            driver_date_utils::get_driver_commit_date_from_github(&commit_sha)
                .unwrap_or(UNKNOWN.into()),
            commit_sha,
        )
    } else {
        (
            driver_date_utils::get_crate_release_date(DRIVER_PKG_NAME, &driver_version)
                .unwrap_or(UNKNOWN.into()),
    ...
    fs::write(
        ...
        format!(
            "pub const PKG_VERSION: &str = \"{}\";\n\
             ...
             pub const DRIVER_VERSION: &str = \"{}\";\n\
             pub const DRIVER_RELEASE_DATE: &str = \"{}\";\n\
             pub const DRIVER_SHA: &str = \"{}\";\n",
...
fn main() {
    ...
    print_version(out_dir, commit_date, sha, lockfile);
}
